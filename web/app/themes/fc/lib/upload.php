...
class CustomPostImages {
	...
	private $cpi_post_types = array('post'); //limit meta box to certain post types
			...
			<div class="cpi-upload-thumbnail">
				...
				<?php
					...
					if( isset($cpi_stored_meta[$cpi_type_name] )) {
						$src = wp_get_attachment_image_src( $cpi_stored_meta[$cpi_type_name][0] );
						echo '<img width="150" height="150" src="'.$src[0].'" />';
					}
	...
	public function cpi_image_enqueue() {
        ...
        if ( in_array( $typenow, $this->cpi_post_types )) {
			...
			wp_register_script( 'cpi-meta-box-image', Assets\asset_path('scripts/admin.js'), array( 'jquery' ) );
	...
	public function cpi_admin_styles() {
        ...
        if ( in_array( $typenow, $this->cpi_post_types )) {
			wp_enqueue_style( 'cpi_meta_box_styles', Assets\asset_path('styles/admin.css') );
		}
	}    
}
